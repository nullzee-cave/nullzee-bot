...
class Useless_commands(commands.Cog, name='Useless Commands'):
    ...
    @commands.command(aliases = ["failed"])
    async def mistake(self, ctx):
        embed = await Embed(ctx.author, description = f'{ctx.author.mention} had an <a:sparkles:932286365008281640> Unintentional failure <a:sparkles:932286365008281640>').user_colour()
        await ctx.send(embed = embed)
    ...
    async def gn(self,ctx,member: discord.Member):
        if member == ctx.author:
            embed = await Embed(ctx.author, description = f"{ctx.author} wished themselves a good sleep",
                                               url = "https://cdn.discordapp.com/emojis/759485870146584586.png?v=1", color=0x00FF00).set_footer(text="Goodnight!").user_colour()
            await ctx.send(embed = embed)
        else:    
            embed = await Embed(ctx.author, description = f"{ctx.author.mention} wished {member.mention} happy dreams", url ="https://cdn.discordapp.com/emojis/759485870146584586.png?7v=1").user_colour()
            await ctx.send(embed = embed)
    ...
    async def gm(self,ctx,member: discord.Member):
        if member == ctx.author:
            embed = await Embed(ctx.author, description = f"{ctx.author} wished themselves a good morning",
                                               url = "https://cdn.discordapp.com/emojis/759485870146584586.png?v=1", color=0x00FF00).set_footer(text="Good morning!").user_colour()
            await ctx.send(embed = embed)
        else:    
            embed = await Embed(ctx.author, description = f"{ctx.author.mention} blessed {member.mention} with a good morning!", url ="https://cdn.discordapp.com/emojis/759485870146584586.png?7v=1").user_colour()
            await ctx.send(embed = embed)
