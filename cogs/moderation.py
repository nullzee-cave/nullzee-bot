...
class Moderation(commands.Cog, name="Moderation"): # moderation commands, warns, mutes etc.
    ...
    async def ban(self, ctx, user: discord.Member, _time: typing.Optional[TimeConverter], *, reason:str=None):
        ...
        await user.send(f"You were banned from {ctx.guild.name} {f'for `{_time}`' if int_time else ''} {f'for `{reason}`' if reason else 'No reason given'}")
    ...
    @commands.command()
    @commands.has_guild_permissions(manage_messages = True)
    async def kick(self,ctx,user: discord.Member,*,reason:string=None):
        if user.guild_permissions.manage_messages:
