...
class level(commands.Cog, name="levelling"):
    ...
    async def on_message(self, message):
        ...
        #     await message.channel.send("WHY PING")
        if not message.guild:
            ...
            if str(message.channel.id) in self.multipliers:
                ...
                multiplier = 1
            # if message.channel.id == 685782425502220311:
            #     multiplier = 0.5
            # else:
            #     multiplier = 1
            if message.attachments:
    ...
    async def update_data(self, user):
        if not await userColl.find_one({"_id": str(user.id)}):
            await userColl.insert_one({"_id": str(user.id), "experience": 0, "weekly": 0, "level": 1, "last_message": 0})
    ...
    async def add_experience(self, userData, user, exp):
        if time.time() - userData["last_message"] > 30:
            await userColl.update_one({"_id": str(user.id)}, {"$inc": {"experience": exp, "weekly": exp}, "$set": {"last_message": time.time()}})
            return await userColl.find_one({"_id": str(user.id)})
        else:
            return await userColl.find_one({"_id": str(user.id)})
    ...
    async def rank(self, ctx, user:discord.Member=None):
        ...
        embed = discord.Embed(title=f"Level: {str(round(userData['level']))}", description=f"XP: {str(round(userData['experience']))}/{str(round(50 * (round(userData['level']) ** 1.5)))}\nWeekly XP: {str(round(userData['weekly']))}", color=0x00FF00)
    ...
    async def weeklyReset(self, ctx):
        ...
        if (math.trunc(time.time()) + 604800) > users["config"]["week_start"]:
            ...
            await ctx.send(embed=discord.Embed(description="Weekly XP leaderboard was reset", color=discord.Color.blue()))
