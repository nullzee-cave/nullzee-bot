...
class Automod(commands.Cog): # this is for timed punishments, removing warns etc.
    ...
    async def on_message(self, message):
        ...
        for word in config["badWords"]:
            if word.lower() == message.content.lower() or f" {word.lower()} " in message.content.lower() or message.content.lower().startswith(word.lower()) or message.content.lower().endswith(word.lower()):
    ...
    async def delete_warns(self):
        async for warn in moderationColl.find({"active": True}):
            if warn["timestamp"] + (await moderationUtils.get_config())["deleteWarnsAfter"] > time.time():
                await moderationColl.update_one(warn, {"$set": {"active": False}})
