...
async def warn_punishments(ctx, user):
    ...
    if not punishment:
        if int(list(config["punishForWarns"].keys())[-1]) < score:
            return await ctx.invoke(ctx.bot.get_command("ban"), user, 31536000, reason="maximum warning limit exceeded")
...
async def end_log(bot, payload, *, moderator, reason):
    ...
    embed = discord.Embed(title=f"un{payload['type'].capitalize()}", colour=discord.Colour.green()).set_author(name=user, icon_url=user.avatar_url)
